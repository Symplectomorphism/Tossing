...
ieout = [];
% Throw phase
[t,y,te,ye,ie] = ode45(@EoM, [t0, tf], x0, options, par);
...
tstep = 0.01;
tspan = 0:tstep:tf;
x = interp1(tout, yout, tspan, 'spline');
t = tspan';
lent = length(t);
...
%         S(13:16)
        ...
        if par.contact % throw
            ...
            dx(1:3) = Rpo*x(4:6);
            dx(4:6) = 1/mo * ( -phio(1:3) );
            dx(7:15) = reshape(Rpod', 9, 1);
            dx(16:24) = reshape( skewmat( Io \ (-phio(4:6)) )', 9, 1);
            dx(25) = x(26);
            dx(26) = -100*x(26) + 0.1*x(25)*wpohat(2,1)^2;
            dx(27) = x(28);
            dx(28) = -1*x(28) - 2*x(27) - pi*x(25)*x(25)*dot(x(4:6),Rpo(:,3));
            dx(29:31) = Rph*x(32:34);
            dx(32:34) = 1/mh * ( f - phih(1:3) );
            dx(35:43) = reshape(Rphd', 9, 1);
            dx(44:52) = reshape( skewmat( Ih \ (tau - phih(4:6)) )', 9, 1);
        end
    end
    

    function [value,isterminal,direction] = events(t,x,par)
        global S
        % Locate the time when the velocity times the acceleration
        % of the dough in the body z-direction is negative 
        value = sign(x(6))*S(3);     % Detect vz*az = 0
